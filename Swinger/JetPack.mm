...
//  Swinger
//
//  Created by Isonguyo Udoka on 8/23/12.
...
- (id) init {
    self = [super init];
    if (self) {
        sprite = [CCSprite spriteWithFile:@"fuelTank.png"];
...
- (CCSprite *) getSprite {
    return [CCSprite spriteWithFile:@"fuelTank.png"];
}
...
- (void) dealloc {
    CCLOG(@"------------------------------ Jet pack being dealloced");
    [super dealloc];
}
